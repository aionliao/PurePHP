...
class Core {
    ...
    function run(){
        if(phpversion() < 5.6){
            die('程序要求PHP5.6+环境版本，当前环境为PHP' . phpversion() . ',请升级服务器环境');            
        }
        session_start();
    ...
    function startApp($module, $controller, $action, $argv){
        $controller = 'application\\' . ($module != '' ? $module . '\\' : '') . 'controller\\' . ucfirst($controller) . 'Controller';
